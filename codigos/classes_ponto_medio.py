...
def imprimir_tabela(frequencia, frequencia_acumulada, frequencia_relativa):
  """Imprime a tabela de frequência com classes, frequências absoluta, acumulada, relativa e ponto médio.
  ...
  """
  print("-" * 90)
  print(f"{'Classe':<15} | {'Frequência Absoluta':<20} | {'Frequência Acumulada':<20} | {'Frequência Relativa':<20} | {'Ponto Médio':<10}")
  print("-" * 90)
  for classe in sorted(frequencia.keys()):
    freq = frequencia[classe]
    freq_ac = frequencia_acumulada[classe]
    ponto_medio = (classe[0] + classe[1]) / 2  # Calcula o ponto médio da classe
    print(f"{classe[0]:.2f} - {classe[1]:.2f} | {freq:<20} | {freq_ac:<20} | {frequencia_relativa[classe]:<20.2%} | {ponto_medio:<10.2f}")
